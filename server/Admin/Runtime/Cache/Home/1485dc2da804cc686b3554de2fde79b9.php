...
$(function(){
    //显示产品图片
    $.ajax({
        url:"/hadmin.php/Home/GoodsList/ajxgoodsimgs",
        type:"get",
        data:{"gid":"<?php echo ($id); ?>"},
...
<div id="spacemenu">
    ...
    <div id="return1"><a href="/hadmin.php/Home/GoodsList/manage?id=<?php echo ($cid); ?>&page=<?php echo ($page); ?>&kwords=<?php echo ($kwords); ?>">返回上一级</a></div>
</div>
<div class="alterdiv"></div>
<div id="posdiv">
	<table width="94%" border="0" cellspacing="0" cellpadding="0" class="gray" align="center">
  <tr>
    <td height="46" class="linkblue">您当前的位置：<a href="/hadmin.php/Home/ColumnManage#<?php echo ($cid); ?>">栏目管理</a> &gt;&gt; <?php echo ($cname); ?> </td>
  </tr>
</table>
</div>
<div id="maindiv">
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="green">
  <tr>
    ...
    <td width="72%"><?php echo ($modselpcls); ?></td>
  </tr>
  <tr>
  	<td height="40" align="right"><font color="#FF0000">*</font> 标题：</td>
    <td width="72%"><input type="text" name="title" id="title" class="htinputcss" value="<?php echo ($data["title"]); ?>" /></td>
  </tr>
    <tr>
        <td height="40" align="right">价格：</td>
        <td><input type="text" name="price" id="price" class="htinputcss" style="width:50px;" value="<?php echo ($data['price']); ?>" /> 元</td>
    </tr>
    <tr>
        <td height="40" align="right">运费：</td>
        <td><input type="text" name="freight" id="freight" class="htinputcss" style="width:50px;" value="<?php echo ($data['freight']); ?>" /> 元</td>
    </tr>
    <?php if(is_array($oneattr)): $i = 0; $__LIST__ = $oneattr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><input type="hidden" name="attrid[]" value="<?php echo ($vo[id]); ?>">
        <?php if($vo[ftype] == 'select'): ?><tr>
                ...
                <td>
                    <select name="sval[]">
                        ...
                        <?php if(is_array($vo[sub])): $i = 0; $__LIST__ = $vo[sub];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$sub): $mod = ($i % 2 );++$i;?><option value="<?php echo ($sub[id]); ?>" <?php if($sub[id] == $sub[value]): ?>selected="selected"<?php endif; ?> ><?php echo ($sub[name]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                    </select></td>
            </tr>
            ...
            <tr>
                ...
                <td>
                    <?php if(is_array($vo[sub])): $i = 0; $__LIST__ = $vo[sub];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$sub): $mod = ($i % 2 );++$i;?><label><input type="checkbox" name="cval<?php echo ($vo[id]); ?>[]" value="<?php echo ($sub[id]); ?>" <?php if($sub[id] == $sub[value]): ?>checked="checked"<?php endif; ?> /><?php echo ($sub[name]); ?></label>&nbsp;&nbsp;&nbsp;<?php endforeach; endif; else: echo "" ;endif; ?>
                </td>
                ...
                <?php if(is_array($specData)): $i = 0; $__LIST__ = $specData;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><input type="hidden" name="spec_attrid[]" value="<?php echo ($vo['id']); ?>" />
                    ...
                    <div class="spec-row">
                        ...
                        <div class="spec-content">
                            <?php if(is_array($vo['sub'])): $i = 0; $__LIST__ = $vo['sub'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$sub): $mod = ($i % 2 );++$i;?><div class="spec-list">
                                <input type="checkbox" data-id="<?php echo ($vo['id']); ?>" class="spec-checkbox" value="1" checked />
                                <input type="text" name="spec_val_<?php echo ($vo['id']); ?>[]" class="spec-input" value="<?php echo ($sub['value']); ?>" placeholder="请输入值" data-id="<?php echo ($vo['id']); ?>" />
  ...
  <tr>
    ...
    <td colspan="2"><textarea name="bodys" id="bodys" class="ckeditor" cols="45" rows="5"><?php echo ($bodys); ?></textarea></td>
  </tr>
  <tr>
    <td height="40" align="right">&nbsp;</td>
    <td><input type="button" name="button" id="button" value="修改" class="addbtn" onclick="check('/hadmin.php/Home/GoodsList/editgoods?id=<?php echo ($id); ?>&cid=<?php echo ($cid); ?>&action=mod&page=<?php echo ($page); ?>')" />&nbsp;&nbsp;
